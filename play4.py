...
npc_buttons = [pygame.Rect(20, 80 + (i * 50), 300, 40) for i in range(len(Game.player.relationships))]
selected_npc = None
        ...
        if event.type == pygame.QUIT:
            ...
            for i,btn in enumerate(npc_buttons):
                if btn.collidepoint(mouse_pos):
                    selected_npc = Game.player.relationships[i]
                    break
                if selected_npc is not None:
